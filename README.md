...
*   **[Stock ASR Eval (offline harness)](./stock-asr-eval/README.md)**: An offline pipeline that measures how much a TWSE-glossary post-transcribe rephrase improves Gemini's Chinese ASR of Taiwanese stock shows (stock-name recall ~78%→99%). Fetches a YouTube video's audio + captions, transcribes (batch or Live), rephrases against an auto-built glossary, and scores CER / term recall. Offline counterpart to `gemini-live-asr-glossary-compare`. Self-host with your own Gemini key — see its [README](./stock-asr-eval/README.md#setup).
